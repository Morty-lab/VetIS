    ...
    <div class="container-xl px-4 mt-4">
        ...
        <div class="card shadow-none p-3">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="">
                    <nav class="nav nav-borders">
                        <a class="nav-link nav-tab {{ request()->is('monthly-sales') ? 'active' : '' }}"
                            href="#monthly-sales">
            ...
            <div class="card-body">
                <table id="monthlySalesReportsTable">
                    <thead>
                        <tr>
                            <th>Month</th>
                            <th>Total Replenished Stocks</th>
                            <th>Remaining Stocks</th>
                            <th>Products Replenished</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                            $monthlyReports = App\Models\Stocks::getReplenishmentReport();
                        @endphp

                        @foreach ($monthlyReports as $month)
                            <tr>
                                <td>{{ \Carbon\Carbon::parse($month->month)->format('F Y') }}</td>
                                <td>{{ number_format($month->total_replenished_stocks) }}</td>
                                <td>{{ number_format($month->remaining_stocks) }}</td>
                                <td>{{ $month->products_replenished }}</td>
                                <td>
                                    <a class="btn btn-datatable btn-primary px-5 py-3"
                                        href="{{ route('reports.replenishment.print', ['date' => $month->month]) }}"
                                        target="_blank">
