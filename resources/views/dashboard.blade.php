...
<script>
    ...
    var salesData = @json($dailySales);  // Pass sales data from controller

    // Prepare labels and data for the chart
    var labels = salesData.map(sale => {
        var date = new Date(sale.date); // Create a Date object from the sale date
        var options = { month: 'short' }; // Get short month name (e.g., 'Jan', 'Feb')
        return date.toLocaleString('en-US', options); // Format the date as month
    });  // or use month for monthly sales
    var data = salesData.map(sale => sale.total_sales); // You can use total_sales or items_sold
    ...
    var mySalesChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: {
                    time: {
                        unit: 'month' // Can be 'day', 'week', 'month' depending on your dataset
                    },
                    gridLines: {
                        display: false,
                        drawBorder: false
                ...
                y: {
                    ticks: {
                        ...
                        callback: function(value) {
                            return "$" + number_format(value, 2, '.', ',');  // Format y-axis labels as currency
                        }
                    },
                    gridLines: {
            ...
            tooltips: {
                ...
                callbacks: {
                    label: function(tooltipItem, chart) {
                        ...
                        return datasetLabel + ": $" + number_format(tooltipItem.raw, 2, '.', ',');  // Format tooltips as currency
                    }
                }
            }
        }
